...
const generatePost = async (business) => {
  ...
  const niche = business.niche || "gym";

  const prompt = `
You are a social media manager for a local ${niche} in India.

Business Details:
- ${niche.charAt(0).toUpperCase() + niche.slice(1)} Name: ${business.name}
- City: ${business.city}
- Owner/Manager Name: ${business.owner_or_manager_name || business.trainer_name}
